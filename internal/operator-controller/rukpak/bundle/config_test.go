...
func Test_UnmarshalConfig(t *testing.T) {
	for _, tc := range []struct {
	...
	}{
		{
			...
			expectedConfig:        nil,
			expectedErrMessage:    "watchNamespace is required for SingleNamespace install mode",
		},
		{
		...
		},
		{
			...
			rawConfig:             []byte(`{"watchNamespace": null}`),
			installNamespace:      "not-some-namespace",
			expectedErrMessage:    "watchNamespace is required for SingleNamespace or OwnNamespace install modes",
		},
		{

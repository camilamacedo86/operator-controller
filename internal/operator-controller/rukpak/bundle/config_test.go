...
func Test_UnmarshalConfig(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
			...
			rawConfig:             nil,
			expectedErrMessage:    "invalid configuration: required field \"watchNamespace\" is missing",
		},
		{
		...
		},
		{
			...
			supportedInstallModes: []v1alpha1.InstallModeType{v1alpha1.InstallModeTypeSingleNamespace},
			rawConfig:             []byte(`true`),
			expectedErrMessage:    `invalid configuration: input is not a valid JSON object`,
		},
		{
			name:                  "rejects additional fields",
			supportedInstallModes: []v1alpha1.InstallModeType{v1alpha1.InstallModeTypeSingleNamespace},
			rawConfig:             []byte(`somekey: somevalue`),
			expectedErrMessage:    `invalid configuration: unknown field "somekey"`,
		},
		{
			name:                  "rejects valid json but invalid registry+v1",
			supportedInstallModes: []v1alpha1.InstallModeType{v1alpha1.InstallModeTypeSingleNamespace},
			rawConfig:             []byte(`{"watchNamespace": {"hello": "there"}}`),
			expectedErrMessage:    `invalid configuration: invalid value type for field "watchNamespace": expected "string" but got "object"`,
		},
		{
			name:                  "rejects with unknown field when install modes {AllNamespaces}",
			supportedInstallModes: []v1alpha1.InstallModeType{v1alpha1.InstallModeTypeAllNamespaces},
			rawConfig:             []byte(`{"watchNamespace": "some-namespace"}`),
			expectedErrMessage:    "invalid configuration: unknown field \"watchNamespace\"",
		},
		{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			config, err := bundle.UnmarshalConfig(tc.rawConfig, schema, tc.installNamespace)
...
// Test_UnmarshalConfig_EmptySchema tests when a bundle doesn't provide a configuration schema.
func Test_UnmarshalConfig_EmptySchema(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			schema, err := emptySchemaBundle.Get()
			require.NoError(t, err)

			config, err := bundle.UnmarshalConfig(tc.rawConfig, schema, "my-namespace")
...
// Test_UnmarshalConfig_HelmLike proves validation works the same for ANY bundle type.
//
...
//
// Same validation process regardless of bundle type.
func Test_UnmarshalConfig_HelmLike(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
			...
			expectedErrMessage: "invalid configuration: required field \"requiredField\" is missing",
		},
		{
		...
		t.Run(tc.name, func(t *testing.T) {
			// Create a mock Helm bundle (real Helm would read values.schema.json)
			helmBundle := &mockHelmBundle{schema: tc.helmSchema}
			schema, err := helmBundle.Get()
			require.NoError(t, err)
...
// Get returns the schema (in real Helm, read from values.schema.json).
func (h *mockHelmBundle) Get() (map[string]any, error) {
	if h.schema == "" {
		return nil, nil
	}
...
// mockEmptySchemaBundle represents a bundle that doesn't provide a configuration schema.
type mockEmptySchemaBundle struct{}

func (e *mockEmptySchemaBundle) Get() (map[string]any, error) {
	return nil, nil
}

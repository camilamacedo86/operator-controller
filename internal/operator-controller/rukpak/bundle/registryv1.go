...
// Get builds a validation schema based on what install modes the operator supports.
//
...
func (rv1 RegistryV1) Get() (map[string]any, error) {
...
func selectNamespaceFormat(hasOwnNamespace, hasSingleNamespace bool) string {
	if hasOwnNamespace && !hasSingleNamespace {
		return formatOwnNamespaceInstallMode
	}
	if hasSingleNamespace && !hasOwnNamespace {
		return formatSingleNamespaceInstallMode
	}
